...
const GlobalPagination: React.FC<PaginationProps> = ({
      ...
      if (
        ...
        pageNumbers.push(
          <button
            ...
            className={` border-y cursor-pointer px-3 py-1 text-sm font-medium ${
              currentPage === i
  ...
  return (
    <nav
      className={cn(
        "flex flex-col items-center justify-between px-1 py-3 sm:flex-row sm:justify-normal sm:px-0",
        `${totalItems === 0 && "hidden"}`
      )}
    >
      <div className="flex flex-col sm:flex-1 sm:flex-row md:flex-col lg:flex-row sm:items-center sm:justify-between justify-center items-center">
        <div className="flex items-center gap-4 justify-between sm:mb-0">
          <p className="text-sm text-gray">
            Showing {startRange}-{endRange} | Page{" "}
        ...
        <div className="mt-4 space-x-1 sm:mt-0">
          <nav
            className="isolate divide-x inline-flex  rounded-md"
            aria-label="Pagination"
          >
            <button
              onClick={() => handlePageChange(Math.max(1, currentPage - 1))}
              disabled={currentPage === 1}
              className={`relative border inline-flex items-center rounded-l-md px-2 py-2 text-gray ${
                currentPage === 1
                  ? "cursor-not-allowed"
            ...
            <button
              ...
              className={`relative border inline-flex items-center rounded-r-md px-2 py-2 text-gray ${
